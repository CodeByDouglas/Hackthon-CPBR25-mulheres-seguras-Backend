...
def delete_contact(contact_id):
    ...
    try:
        ...
        db.session.commit()
        return jsonify({"success": True, "message": "Contato removido com sucesso"}), 200
    except Exception as e:
        db.session.rollback()
        return jsonify({"success": False, "error": f"Erro ao remover contato: {str(e)}"}), 500
...
def auto_emergency(token):
    ...
    try:
        ...
        for contato in contatos:
            ...
            mensagem = (
                f"ALERTA! {user.nome} precisa de ajuda. Acesse: {link}"
            )
            try:
